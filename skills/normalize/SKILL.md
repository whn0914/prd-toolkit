...
- **「四、流程」**：必须生成流程图。若原文档有文字流程描述，将其转为 `flowchart TD`；若原文档已有流程图（截图），标注 `[原文档含流程图，需 PM 确认 Mermaid 是否准确]` 并尝试还原
- **「五、产品/交互逻辑」**：存在多状态流转或复杂分支时，插入对应 `flowchart TD` 或 `stateDiagram-v2`
- 原文档中无足够信息生成流程图时，标注 `（原文档流程描述不足，需 PM 补充后完善）`
